...
This script aims to train a classifier based on representations extracted from a pretrained resNet. The goal is to assess the performance of the classifier when no reweighting is done on the dataset.
"""
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=15, start_epoch=0, val_mode=False,
                show_progress=False):
    if show_progress:
        vis = visdom.Visdom()
...
W_PROTECTED, BIAS, VAL_MODE, START_EPOCH, NUM_EPOCH, SHOW_PROGRESS, ID = 1, 0, False, 0, 15, False, 0

for opt, arg in opts:
    if opt == '-h':
        print("Case_1+2.py --w_protected=<w_protected> --bias=<bias> --val_mode=<val_mode> --start_epoch=<start_epoch>"
              "--num_epoch=<num_epoch> --visdom=<wisdom> --id=<id> --num_trials=<num_trials>")
        sys.exit()
    if opt == '--w_protected':
        W_PROTECTED = int(arg)
    if opt == '--bias':
...
print(
    ...
    f"-num_epoch={NUM_EPOCH} -visdom={SHOW_PROGRESS}, -id={ID}, -num_trials={NUM_TRIALS}")
...
image_datasets = {
    x: my_ImageFolder(os.path.join(data_dir, f"train_{BIAS}" if x == "train" and BIAS else x), data_transforms[x],
                      set(dr_f_d + dr_f_l + nur_m_d + nur_m_l), W_PROTECTED)
    for x in ['train', 'test']}
...
for trial in range(NUM_TRIALS):
    ...
    train_accs.append(float(accuracy(model_conv, dataloaders['train'])))
    test_accs.append(float(accuracy(model_conv, dataloaders['test'])))
